...
**Do not chain these jobs.** There is no wrapper that submits everything. Run **one script, then the next**. Two steps are human (cysteines twice). SignalP/DeepTMHMM are optional checks.
...
sbatch --export=ALL,NITR_CONFIG=$NITR_CONFIG steps/09_interproscan.sh
# wait
# bash steps/09w_web_help.sh                      # optional SignalP/DeepTMHMM check
sbatch --export=ALL,NITR_CONFIG=$NITR_CONFIG steps/09b_architecture.sh
# wait
sbatch --export=ALL,NITR_CONFIG=$NITR_CONFIG steps/10_flanks.sh
# wait
