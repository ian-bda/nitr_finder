...
Transcripts that span two NITRs are split by nearest V/I (not skipped).
"""
...
from stitch_nitr_genomic import exons_owned_by
...
def main() -> None:
    ...
    for gene in genes:
        ...
        if rec is None:
            notes.append("no_transcript")
            rows.append(empty_row(gene, ";".join(notes)))
            continue

        rec = dict(rec)
        rec["exons"] = list(rec["exons"])
        if n_genes_hit(rec, genes) > 1:
            notes.append("clipped_fusion")
            rec["exons"] = exons_owned_by(rec["exons"], gene, genes)
            if rec["exons"]:
                rec["start"] = rec["exons"][0][0]
                rec["end"] = rec["exons"][-1][1]
        if not rec["exons"]:
            notes.append("no_local_exons")
            rows.append(empty_row(gene, ";".join(notes)))
            continue
